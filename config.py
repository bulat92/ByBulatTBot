...
def start_message(message):
    if message.text == 'kushnarenkovo': # если кушнаренково
        ...
        bot.send_message(message.chat.id, 'Air temperature in Kushnarenkovo ' + str(w.get_temperature('celsius')["temp"]) + "°C", reply_markup=keyboard2) # поголдду градусы отправь
    
    elif message.text == 'ufa':
        observation = owm.weather_at_place('ufa,RU')
        w = observation.get_weather()
        bot.send_message(message.chat.id,'Air temperature in Ufa ' + str(w.get_temperature('celsius')["temp"]) + "°C" , reply_markup=keyboard2)
