...
keyboard1 = telebot.types.ReplyKeyboardMarkup(True, True) # клавиатуры
...
@bot.message_handler(commands=['start']) #фильтр обработчик
def first_message(message): # имя функций
    bot.send_message(message.chat.id, "Hello you wrote me /start \nWhat city temperature do you want to know?", reply_markup=keyboard1) # бот отправляет в чат ответ и вывыводит клаву
